...
//System.out.println("Score: " + newEval);
        ...
        private boolean isCorner(int x, int y) {
            ...
            if (x == Constants.SIZE - 1 && y == 0) {
                return true;
            }
            if (x == 0 && y == Constants.SIZE - 1) {
                return true;
            }
        ...
        private boolean isStable(Board board, int x, int y) {
            ...
            for (String i : directions) {
                if (i.equals("N")) {
                    nStatic = checkDirectionStable(board, new Position(x, y), i);
                }
                if (i.equals("NE")) {
                    neStatic = checkDirectionStable(board, new Position(x, y), i);
                }
                if (i.equals("E")) {
                    eStatic = checkDirectionStable(board, new Position(x, y), i);
                }
                if (i.equals("SE")) {
                    seStatic = checkDirectionStable(board, new Position(x, y), i);
                }
                if (i.equals("S")) {
                    sStatic = checkDirectionStable(board, new Position(x, y), i);
                }
                if (i.equals("SW")) {
                    swStatic = checkDirectionStable(board, new Position(x, y), i);
                }
                if (i.equals("W")) {
                    wStatic = checkDirectionStable(board, new Position(x, y), i);
                }
                if (i.equals("NW")) {
                    nwStatic = checkDirectionStable(board, new Position(x, y), i);
                }
            }
        ...
        private boolean checkDirectionStable(Board board, Position position, String direction) {
            ...
            while (true) {
                ...
                if (stableSquares[newPosition.getRow()][newPosition.getCol()] && squaresEqual(originalSquare, board.getSquare(newPosition))) {
                    return true;
                }
                if (board.getSquare(newPosition).getStatus() == 0) {
                    return false;
                }
        ...
        private boolean isDirFilled(Board board, Position start, Position vector, Position antiVector) {
            ...
            while (!newPos.isOffBoard()) {
                if (board.getSquare(newPos).getStatus() == 0) {
                    return false;
                }
